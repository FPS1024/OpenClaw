...
final class GatewayClient: ObservableObject {
    ...
    @Published var availableSessions: [SessionEntry] = []

    private var historyCacheKey: String { "chat.cache.\(currentSessionKey)" }
    ...
    func handleScenePhase(_ phase: ScenePhase) {
        ...
        case .active:
            if connectionState == .disconnected || isErrorState {
                ...
                // Connection survived the background — restart keep-alive and
                // pull any messages that arrived while we were suspended.
                if keepAliveEnabled {
                    startKeepAliveLoop()
                }
    ...
    func abortCurrentRun() {
        ...
        endStreaming()
        Task {
            _ = try? await sendRequest(method: "chat.abort", params: ["sessionKey": currentSessionKey])
        }
    }
    ...
    func switchToSession(_ key: String) {
        guard key != currentSessionKey else { return }
        endStreaming()
    ...
    private func refreshHistory() async {
        ...
        if !history.isEmpty {
            messages = history
            // History reflects the settled server state; any streaming run that
            // finished while we were in the background is already included, so
            // clear stale streaming bookkeeping.
            activeRunMessageIds.removeAll()
            isStreaming = false
            persistMessages()
        }
    ...
    private func fetchSessions() async {
        ...
        guard let response = try? await sendRequest(method: "sessions.list", params: params),
              let payload = response["payload"] as? [String: Any],
              let list = payload["sessions"] as? [[String: Any]] else { return }
        availableSessions = list.compactMap { dict in
            guard let key = dict["key"] as? String else { return nil }
            let label = dict["label"] as? String ?? dict["displayName"] as? String
            return SessionEntry(key: key, label: label)
        }
    }
